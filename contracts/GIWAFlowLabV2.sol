...
contract GIWAFlowLabV2 {
    ...
    // =============================================================
    // V2 decimal-aware mock units
    // =============================================================
    //
    // Solidity uses integers only. V2 stores each mock asset in its
    ...
    uint256 public constant MGIWA_UNIT = 1e18;
    uint256 public constant MUSD_UNIT = 1e18;
    uint256 public constant MBTC_UNIT = 1e8;
    uint256 public constant MBTC_PRICE_MUSD = 100_000;
    ...
    uint256 public constant MAX_LP_VALUE_PER_WALLET_POOL = 5_000 * MUSD_UNIT;
    uint256 public constant MIN_LP_VALUE = 100 * MUSD_UNIT;
    ...
    function depositNative() external payable nonReentrant whenDepositsActive {
        ...
        uint256 totalEligibleMock = ((uint256(account.lifetimeDeposited) * MOCK_MINT_RATE) / 1 ether) * MGIWA_UNIT;
    ...
    uint256 public constant DAILY_LOGIN_REWARD = 25 * MGIWA_UNIT;
    uint256 public constant EXTRA_WHEEL_COST = 25 * MGIWA_UNIT;
    ...
    function _mockRandomReward(
        ...
        return (minReward + (randomValue % spread)) * MGIWA_UNIT;
    }
    ...
    uint256 public constant SCRATCH_COST = 50 * MGIWA_UNIT;
    ...
    function _toMUSDValue(uint8 token, uint256 amount) internal pure returns (uint256) {
        ...
        if (token == uint8(MockToken.mBTC)) {
            return (amount * MBTC_PRICE_MUSD * MUSD_UNIT) / MBTC_UNIT;
        }
    ...
    function _fromMUSDValue(uint8 token, uint256 valueMUSD) internal pure returns (uint256) {
        ...
        if (token == uint8(MockToken.mBTC)) {
            return (valueMUSD * MBTC_UNIT) / (MBTC_PRICE_MUSD * MUSD_UNIT);
        }
    ...
    function _poolWeeklyEmissionCap(uint8 pairType) internal pure returns (uint256) {
        if (pairType == uint8(PairType.mGIWA_mUSD)) return 10_000 * MGIWA_UNIT;
        if (pairType == uint8(PairType.mGIWA_mBTC)) return 15_000 * MGIWA_UNIT;
        if (pairType == uint8(PairType.mUSD_mBTC)) return 8_000 * MGIWA_UNIT;
